...
export function FilterBar({ onCreate }: { onCreate: (presensi: NewPresensiGuru & { session: string }) => void }) {
  ...
  return (
    <div className="flex w-full flex-col items-stretch gap-4 rounded-xl bg-card p-5 shadow-[0px_4px_10px_0px_rgba(0,0,0,0.03)] sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
        <label className="flex items-center rounded-lg border border-line bg-field px-[13px] py-[9px]">
          <span className="sr-only">Tanggal dan waktu presensi</span>
          <input
            type="datetime-local"
            className="w-full min-w-0 bg-transparent text-[14px] leading-5 text-ink outline-none sm:w-auto"
          />
        </label>
        ...
        <label className="flex items-center rounded-lg border border-line bg-field px-[13px] py-[9px]">
          ...
          <select
            ...
            className="w-full min-w-[100px] bg-transparent text-[14px] leading-5 text-ink outline-none sm:w-auto"
          >
