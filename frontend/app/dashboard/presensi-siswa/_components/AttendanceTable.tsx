          ...
          <tbody>
            {RECORDS.map((record) => (
              <tr key={record.no} className="border-b border-divider last:border-b-0">
                ...
                <td className="px-6 py-2.5 text-[14px] leading-5 text-ink whitespace-nowrap">{record.studentName}</td>
                <td className="px-6 py-2.5 text-[14px] leading-5 text-muted whitespace-nowrap">{record.halaqah}</td>
                <td className="px-6 py-2.5 text-[14px] leading-5 text-ink whitespace-nowrap">{record.date}</td>
                <td className="px-6 py-2.5 text-[14px] leading-5 text-muted whitespace-nowrap">{record.session}</td>
                <td className="px-6 py-2.5">
                  <StatusBadge status={record.status} />
                </td>
                <td className="px-6 py-2.5 text-[14px] leading-5 text-muted">{record.note}</td>
              </tr>
            ))}
          </tbody>
        </table>
